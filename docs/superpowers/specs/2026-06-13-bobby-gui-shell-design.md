...
> 日期：2026-06-13（v2 修订：双入口移至侧边栏顶部分段切换；Loop 确立"引导式撰写"方向；视觉语言定稿 Deep Glow）
> 状态：待用户评审
...
│ ┌──────────────────┐ │ ┌─────────────────────────────┐ │ ┌──┬──┬──┬──┐      │
│ │ [💬 Chat][</>Code]│ │ │ 会话标题 · ⎇main   [摘要栏▾]│ │ │审│终│网│SC│      │
│ └──────────────────┘ │ ├─────────────────────────────┤ │ ├──┴──┴──┴──┤      │
│  ＋ New Chat         │ │                             │ │ │            │      │
│  🔍 搜索             │ │  对话流                     │ │ │ 文件变更    │      │
│  ✍  Write           │ │  · 用户消息                 │ │ │ diff 视图   │      │
...
│   · 条目…(状态点)    │ │ │ [模型▾][权限▾][思考] ⏎  │ │ │ │            │      │
│ ──────────────────── │ │ └─────────────────────────┘ │ │ └────────────┘      │
│  ⚙ Settings  👤 账号 │ │ cache 92%·12.3k tok·¥0.18  │ │                    │
├──────────────────────┴─┴───────────────────────────────┴────────────────────┤
│ ⑤ 状态栏  kernel ● 已连接 · DeepSeek ● Flash · 上下文 34% · v0.1.0 · ⬆更新   │
└─────────────────────────────────────────────────────────────────────────────┘
```

五个区域，每个都是独立组件，互相只通过全局 store 通信：

| # | 区域 | 职责 |
|---|---|---|
| ① | 标题栏 | 项目切换器、全局搜索入口（Ctrl+K 命令面板）、通知铃铛、窗口控制 |
| ② | 侧边栏 Sidebar | **顶部分段切换器：Chat / Code**（CC desktop 同款，分段条上可显示待审批角标，未来可扩第三模式）；中部随模式切换：固定功能项 + 列表（置顶/项目/记录，CX）；**底部：Settings、账号** |
| ③ | 工作区 Workspace | 顶部会话标题+摘要栏（CX）、中部对话流、底部 composer + 计费条（DS） |
| ④ | 右侧面板 | 可折叠 tabs：审查 / 终端 / 网页 / SideChat（CX） |
| ⑤ | 状态栏 | kernel 连接、模型、上下文占用、版本、更新提示 |

### 1.1 视觉语言：Deep Glow（沉浸式驾驶舱）

目标体验是**沉浸式 vibecoding**：界面像一台深海探测器的驾驶舱 —— 极深的底色让内容发光，数据有仪器感，动效克制但处处有呼吸。具体 tokens：

- **底色**：黑曜石深蓝绿（`#04070b` 系），不用纯黑；窗口背景叠两层极淡的径向光晕（左上青、右下靛）+ 3% 噪点颗粒，制造深度而非平面。
- **材质**：面板用玻璃拟态（`backdrop-blur` + 半透明 + 1px 发丝高光边框），层级靠模糊与透明度区分，不靠粗边框。
- **主强调色**：生物荧光青 `#36e4c8`（Bobby 的"光"）—— 用于激活态、运行中、证据、品牌；激活元素带柔和辉光（glow shadow）。闸口审批用琥珀、错误用珊瑚红，**不出现紫色渐变**。
- **字体**：品牌/数字仪表用 Chakra Petch（科技感无机械感），正文 IBM Plex Sans（中文回退 MiSans/微软雅黑），代码与证据 IBM Plex Mono。计费、token、百分比一律用 mono + 青色，强化"仪表读数"的心智。
- **动效**：一次性的页面载入交错浮现（stagger reveal）；运行中状态点呼吸脉冲；分段切换器滑动指示器；tab 下划线滑动；卡片 hover 微抬升 + 辉光。原则：动效服务于"系统在活着"的感知，不做装饰性长动画。
- **空间**：信息密度向专业工具看齐（紧凑行高），但卡片间留足呼吸；对话流限宽，证据/工具卡片通栏。

这套 tokens 在 P0 阶段就落成 `src/lib/theme.css` 的 CSS variables，是外壳验收的一部分。
...
1. **闸口审批 UI（GateDialog 升级）** —— Bobby 的核心是"不撒谎+闸口"。审批不能只是弹窗：
   ...
   - 全局"待审批"徽标：其他会话有 pending gate 时，侧边栏顶部分段切换器的 Code 分段和会话列表条目上显示 ◐ 角标，点击直达。
...
### 3.1 模式切换器（ModeSwitch，侧边栏顶部）

CC desktop 同款的分段胶囊控件，置于侧边栏最顶部：`[💬 Chat] [</> Code]`，滑动指示器动画切换。徽标规则：任何后台会话出现 pending gate / 失败，对应分段右上角显示数字角标。控件按"可扩展分段"实现（数组渲染），未来加第三模式（如 Cowork）零结构改动。

### 3.2 侧边栏（Sidebar）

宽 280px，可拖拽调宽、可折叠。结构三段式：

- **顶部**：模式切换器（§3.1）。
- **功能项区**（固定，随模式切换）：见 §2 导航树。每项 = 图标 + 文案；未实现的模块照常可点，进入占位页，未上线项带「即将推出」小标。
...
**Loop 的创建必须是引导式的（用户明确要求，最终形态）**：loop 的四个部分对撰写质量要求远高于普通对话 prompt，不能给一个自由文本框了事。创建流程做成**分步向导**：每步聚焦一个字段，提供 1) 该字段的写作要点说明与反例，2) 模板/预设（如验收器预设：测试通过 / lint 零警告 / Pro 评分阈值），3) **AI 辅助润写**——用户写草稿，Flash 按该字段的质量标准改写并解释改动，用户确认后落定。最后一步整体预览 + 试运行一轮（dry-run）。UI-first 阶段：占位页先展示四步向导的骨架示意即可，引导逻辑属于 P5。
...
│  ├─ TitleBar.tsx  ModeSwitch.tsx  Sidebar.tsx
...
- **样式**：继续纯 CSS + CSS variables 设计 tokens，按 §1.1 Deep Glow 视觉语言落成 `lib/theme.css`（暗色优先）。不引组件库，保持 Bobby 自己的脸。字体随应用打包（Chakra Petch / IBM Plex），不依赖网络字体。
