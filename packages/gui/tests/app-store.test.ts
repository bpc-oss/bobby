...
function statesForFinalResult(sampleEvidence: Evidence): ReturnType<typeof reduce>[] {
  const baseState = {
    ...initial(),
    pendingGate: { gateId: 'gate-1', reason: 'manual' },
    steps: ['step-1'],
    evidence: [sampleEvidence]
  };

  return [
    reduce(baseState, { type: 'final_result', taskId: 'task-1', status: 'done' }),
    reduce(baseState, { type: 'final_result', taskId: 'task-1', status: 'failed' }),
    reduce(baseState, { type: 'final_result', taskId: 'task-1', status: 'blocked' })
  ];
}
...
it('updates status and clears pending gate when final_result arrives', () => {
  const states = statesForFinalResult(sampleEvidence);
